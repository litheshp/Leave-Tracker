dataApp.controller('AlldataController',				[
						...
						function($scope, $resource, $http) {
							...
							//var calCulatedata = $resource('/calculate/getConfig');
							var calCulatedata = $resource('/config/list');
							calCulatedata.query(function(results) {
								...
								*/
								 ...
								 $scope.WeekList = getUniqueData(results[0].WeekList);
									$scope.empIdlist = (results[0].empIdlist);
									$scope.empList = (results[0].empList);
									$scope.LocationList = (results[0].LocationList);
									$scope.ProjectList = (results[0].ProjectList);
									$scope.TeamList = (results[0].TeamList);
									$scope.TeamData =results[0].teamData;
								var len = $scope.WeekList.length;								
								$scope.latestWeek =$scope.WeekList[len-1];
							...
							$scope.byTeam = function() {
								...
								document.getElementById("overlay").style.display = "block";
								for(i=0;i<$scope.ProjectList.length;i++){
									if($scope.Project == $scope.ProjectList[i] ){
										//alert($scope.ProjectList[i])
										$scope.TeamList = $scope.TeamData[i]
									}
							...
							$scope.calculateEmployeeData = function(data) {
								...
								.success(
										function(results) {
											...
											for (i = 0; i < $scope.empIdlist.length; i++) {
												for (j = 0; j < results.length; j++) {
													if (results[j].Emp_ID == empIdlist[i]) {
														...
														Site = results[j].Site;
														Available_Hrs = Available_Hrs
																+ results[j].Available_Hrs;
														Productive_Hrs = Productive_Hrs
																+ results[j].Productive_Hrs;
														OT_Hours = OT_Hours
																+ results[j].OT_Hours;
														Leave_Hrs = Leave_Hrs
																+ results[j].Leave_Hrs;
														Chargeable_Hrs = Chargeable_Hrs
																+ results[j].Chargeable_Hrs;
														DH = DH + results[j].DH;
													}
												}
